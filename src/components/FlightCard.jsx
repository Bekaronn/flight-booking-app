...
function FlightCard({ offer }) {
  ...
  const secondItinerary = itineraries[1] || null;
  ...
  const returnDuration = secondItinerary?.duration?.replace("PT", "").toLowerCase();
            ...
            {itineraries.map((itin, idx) => (
              <div key={idx} className="mb-4">
                ...
                {itin.segments?.map((seg, i) => (
                  ...
                  >
                    <p>
                      ✈️ <b>{seg.carrierCode}</b> — {seg.departure.iataCode} → {seg.arrival.iataCode}
                    </p>
                    <p>
